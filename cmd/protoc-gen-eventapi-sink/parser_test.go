...
func TestNormalizeHeaderPath(t *testing.T) {
	tests := []struct {
		...
		// Default: auto-add ce_ prefix
		{"trace_id", "trace_id", "ce_trace_id"},
		{"id", "id", "ce_id"},
		{"time", "time", "ce_time"},
		{"source", "source", "ce_source"},
		{"type", "type", "ce_type"},
		{"subject", "subject", "ce_subject"},
		{"span_id", "span_id", "ce_span_id"},
		// Already has ce_ prefix
		{"ce_id", "ce_id", "ce_id"},
		{"ce_trace_id", "ce_trace_id", "ce_trace_id"},
		{"ce_time", "ce_time", "ce_time"},
		// raw: prefix to skip auto-prefix
		{"raw:x-custom", "raw:x-custom", "x-custom"},
		{"raw:trace_id", "raw:trace_id", "trace_id"},
		// Nested JSON paths (unchanged)
		{"nested path", "$.ce_source.tenant", "$.ce_source.tenant"},
		{"nested custom", "$.custom_field.sub", "$.custom_field.sub"},
		// Edge cases
		{"empty string", "", ""},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := normalizeHeaderPath(tt.input)
			if result != tt.expected {
				t.Errorf("normalizeHeaderPath(%q) = %q, want %q", tt.input, result, tt.expected)
			}
		})
	}
}
...
func TestParseSinkRule(t *testing.T) {
	...
	if len(sink.Mappings) != 3 {
		t.Fatalf("Mappings count = %d, want 3 (2 explicit + 1 auto event_time)", len(sink.Mappings))
	}
	if !sink.Mappings[0].IsHeader {
		t.Error("first mapping should be header (event_id)")
	}
	if !sink.Mappings[1].IsHeader {
		t.Error("second mapping should be header (event_time)")
	}
	if !sink.Mappings[2].IsData {
		t.Error("third mapping should be data (user_id)")
	}
}
...
func TestParseSinkRuleEmpty(t *testing.T) {
	...
	// autoMapStandardHeaders adds event_id and event_time automatically
	if len(sink.Mappings) != 2 {
		t.Errorf("Mappings count = %d, want 2", len(sink.Mappings))
	}
}
...
func TestInferTypeHeader(t *testing.T) {
	tests := []struct {
		headerPath string
		want       string
	}{
		{"ce_time", "COLUMN_TYPE_TIMESTAMP"},
		{"time", "COLUMN_TYPE_TIMESTAMP"},
		{"ce_id", "COLUMN_TYPE_VARCHAR"},
		{"id", "COLUMN_TYPE_VARCHAR"},
		{"ce_type", "COLUMN_TYPE_VARCHAR"},
		{"trace_id", "COLUMN_TYPE_VARCHAR"},
		{"ce_trace_id", "COLUMN_TYPE_VARCHAR"},
		{"raw:x-custom", "COLUMN_TYPE_VARCHAR"},
	}
...
func TestInferTypeUnknownHeaderDefaultsToVarchar(t *testing.T) {
	m := MappingInfo{
	...
	if got != "COLUMN_TYPE_VARCHAR" {
		t.Errorf("inferType = %s, want COLUMN_TYPE_VARCHAR", got)
	}
}
...
func TestInferTypeUnspecifiedDataPathError(t *testing.T) {
	m := MappingInfo{
		DataPath: "$.unknown_field",
		IsData:   true,
	}
	_, err := inferType(m, nil)
	if err == nil {
		t.Fatal("expected error for unknown data path, got nil")
	}
}
