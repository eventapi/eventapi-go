...
//   3. Header 字段: ce_time -> TIMESTAMP, 其他 -> VARCHAR
...
func inferType(m MappingInfo, protoFields map[string]ProtoFieldInfo) (string, error) {
	...
	// 3. Header 字段: ce_time -> TIMESTAMP, 其他 -> VARCHAR
	if m.IsHeader {
		if normalizeHeaderPath(m.HeaderPath) == "ce_time" {
			return "COLUMN_TYPE_TIMESTAMP", nil
		}
		return "COLUMN_TYPE_VARCHAR", nil
	}
...
// normalizeHeaderPath 标准化 header_path：
//   - "trace_id" -> "ce_trace_id"（默认补 ce_ 前缀）
//   - "id" -> "ce_id"（标准头也补 ce_ 前缀）
//   - "ce_trace_id" -> "ce_trace_id"（已有前缀，不变）
//   - "raw:x-custom" -> "x-custom"（raw: 前缀跳过自动补全）
...
func normalizeHeaderPath(path string) string {
	...
	if strings.HasPrefix(path, "raw:") {
		return strings.TrimPrefix(path, "raw:")
	}
	if strings.HasPrefix(path, "ce_") {
		return path
	}
	return "ce_" + path
}
...
func parseSinkRule(method *protogen.Method, rule *eventapiv1.SinkRule, file *protogen.File) (SinkInfo, error) {
	...
	// 自动映射 event_id 和 event_time (如果用户未显式指定)
	sink.Mappings = autoMapStandardHeaders(sink.Mappings)

	return sink, nil
}

// autoMapStandardHeaders 自动添加标准头映射 (如果用户未显式指定)
func autoMapStandardHeaders(mappings []MappingInfo) []MappingInfo {
	hasEventID := false
	hasEventTime := false
	for _, m := range mappings {
		...
		if m.TargetColumn == "event_time" {
			hasEventTime = true
		}
	}

	result := make([]MappingInfo, 0, len(mappings)+2)
...
func parseMapping(m *eventapiv1.FieldMapping) MappingInfo {
	...
	case *eventapiv1.FieldMapping_HeaderPath:
		info.HeaderPath = normalizeHeaderPath(src.HeaderPath)
		info.IsHeader = true
		// 嵌套 JSON 提取: "$.ce_source.tenant"
		if len(info.HeaderPath) > 2 && info.HeaderPath[:2] == "$." {
			info.IsNestedJSON = true
			info.NestedJSONKey = info.HeaderPath
			// 提取 header 名称: "$.ce_source.tenant" -> "ce_source"
			parts := strings.SplitN(info.HeaderPath[2:], ".", 2)
			info.HeaderName = parts[0]
		} else {
			info.HeaderName = info.HeaderPath
		}
