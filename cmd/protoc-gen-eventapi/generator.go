...
// 如果有多个字段标记为 subject，则 panic 提示用户明确指定
func getSubjectField(msg MessageInfo) string {
	...
	if len(subjects) > 1 {
		panic(fmt.Sprintf("message %s has multiple is_event_subject fields: %v. Please mark only one field as subject.", msg.Name, subjects))
	}
	return subjects[0]
}

// buildExtensions 构建 CloudEvent Extensions 初始化代码
func buildExtensions(msg MessageInfo) string {
	var headers []FieldInfo
	for _, f := range msg.Fields {
		if f.IsHeader {
	...
	for _, h := range headers {
		parts = append(parts, fmt.Sprintf("\"%s\": cloudevent.NewAttributeValue(event.%s)", h.JSONTag, h.Name))
	}
...
func generateClientFile(gen *protogen.Plugin, file *protogen.File, data GeneratorData) error {
	...
	tmplName := fmt.Sprintf("client_%s", strings.ToLower(data.Protocol))
	if err := templates.ExecuteTemplate(g, tmplName, data); err != nil {
		return err
	}
