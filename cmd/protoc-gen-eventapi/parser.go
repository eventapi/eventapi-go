...
type MessageInfo struct {
	Name            string      // 结构体名
	Comment         string      // 注释
	Fields          []FieldInfo // 所有字段列表
	HeaderFields    []FieldInfo // 标记为 is_event_header 的字段
	DataFields      []FieldInfo // 未标记为 header 的字段（Data payload）
	DataContentType string      // 数据编码格式: "CONTENT_TYPE_JSON", "CONTENT_TYPE_PROTOBUF"
}
...
type FieldInfo struct {
	...
	IsHeader  bool   // 是否提取为 CloudEvent header
}
...
func parseMessage(msg *protogen.Message) *MessageInfo {
	...
	for _, field := range msg.Fields {
		...
		f := FieldInfo{
			...
			IsHeader:  isHeaderField(field),
		}
		info.Fields = append(info.Fields, f)
		if f.IsHeader {
			info.HeaderFields = append(info.HeaderFields, f)
		} else {
			info.DataFields = append(info.DataFields, f)
		}
	}

	// 提取 event_data_content_type option
	if msg.Desc.Options() != nil {
		if proto.HasExtension(msg.Desc.Options(), eventapiv1.E_EventDataContentType) {
			dct := proto.GetExtension(msg.Desc.Options(), eventapiv1.E_EventDataContentType).(eventapiv1.EventDataContentType)
			info.DataContentType = dataContentTypeToString(dct)
		}
	}
...
// dataContentTypeToString 将 proto 枚举映射为 CloudEvent 标准 content-type
func dataContentTypeToString(ct eventapiv1.EventDataContentType) string {
	switch ct {
	case eventapiv1.EventDataContentType_CONTENT_TYPE_JSON:
		return "application/json"
	case eventapiv1.EventDataContentType_CONTENT_TYPE_PROTOBUF:
		return "application/x-protobuf"
	case eventapiv1.EventDataContentType_CONTENT_TYPE_AVRO:
...
// isSubjectField 检查字段是否有 (eventapi.v1.is_event_subject) = true
func isSubjectField(field *protogen.Field) bool {
	if field.Desc.Options() != nil {
		return proto.GetExtension(field.Desc.Options(), eventapiv1.E_IsEventSubject).(bool)
	}
	return false
}

// isHeaderField 检查字段是否有 (eventapi.v1.is_event_header) = true
func isHeaderField(field *protogen.Field) bool {
	if field.Desc.Options() != nil {
		return proto.GetExtension(field.Desc.Options(), eventapiv1.E_IsEventHeader).(bool)
	}
	return false
}
