...
// WriteFunc 底层写入操作。
type WriteFunc func(context.Context, *kafkago.Message) error

// WriterMiddleware 发送中间件，可观察/修改最终写入 Kafka 的消息。
type WriterMiddleware func(next WriteFunc) WriteFunc

// FetchFunc 底层拉取操作。
type FetchFunc func(context.Context) (*kafkago.Message, error)

// ReaderMiddleware 接收中间件，可观察/修改从 Kafka 读取的消息。
type ReaderMiddleware func(next FetchFunc) FetchFunc
...
func New(_ context.Context, cfg Config) (Transport, error) {
	...
	return &kafkaTransport{
		config:           cfg,
		writer:           writer,
		writerMiddleware: cfg.WriterMiddleware,
	}, nil
}
...
func (c *kafkaChannel) Send(ctx context.Context, event *cloudevent.CloudEvent[[]byte]) error {
	...
	writeFunc := func(ctx context.Context, m *kafkago.Message) error {
		return c.transport.writer.WriteMessages(ctx, *m)
	}
	for i := len(c.transport.writerMiddleware) - 1; i >= 0; i-- {
		writeFunc = c.transport.writerMiddleware[i](writeFunc)
	}

	return writeFunc(ctx, msg)
}
...
func (c *kafkaChannel) receiveLoop(ctx context.Context, handle func(context.Context, *ReceivedEvent[[]byte]) error) error {
	...
	for {
		...
		fetchFunc := func(ctx context.Context) (*kafkago.Message, error) {
		...
		for i := len(c.readerMiddleware) - 1; i >= 0; i-- {
			fetchFunc = c.readerMiddleware[i](fetchFunc)
		}
